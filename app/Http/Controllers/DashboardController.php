...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        {
            $title = 'Super Admin Dashboard';
            return view('backend.super_admin_dashboard',compact('title'));
        }
        elseif(Auth::user()->role_types_id == 2)
        {
            $title = 'Admin Dashboard';
            return view('backend.admin_dashboard',compact('title'));
        }
        elseif(Auth::user()->role_types_id == 3)
        {
            $title = 'Employee Dashboard';
            return view('backend.employee_dashboard',compact('title'));
        }
        else
        {
            $title = 'Company Dashboard';
            return view('backend.company_dashboard',compact('title'));
        }
    }
}
